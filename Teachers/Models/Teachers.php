...
class Teachers extends Connection{
	...
	public function add($firstname, $lastname, $user, $password){
		$statement = $this->db->prepare("INSERT INTO USUARIOS (NOMBRE, APELLIDO, USUARIO, PASSWORD, PERFIL, ESTADO) VALUES (:Nombre, :Apellido, :Usuario, :Password, Teacher, Active)");
	...
	public function get(){
		$rows = NULL;
		$statement = $this->bd->prepare("SELECT * FROM USUARIOS WHERE PERFIL = Teacher");
		$statement->execute();
	...
	public function getById($id){
		$rows = NULL;
		$statement = $this->bd->prepare("SELECT * FROM USUARIOS WHERE PERFIL = Teacher AND ID_USUARIO = :Id");
